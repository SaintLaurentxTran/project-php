...
<section class="container pt-32">
  <div class="seller-head">
    <h1><?= e($formTitle) ?></h1>
    <a class="btn" href="<?= e(url('seller', 'categories')) ?>">Quay lại</a>
  </div>
  ...
  <form class="grid2" method="POST" action="<?= e($formAction) ?>">
    ...
    <div class="card">
      ...
      <input class="input" name="icon" maxlength="64"
             ...
             placeholder="Ví dụ: checkroom, devices, chair">

      <p class="muted small mt-8">Tên icon sử dụng bộ Material Symbols đang có trên website.</p>
    </div>
    ...
    <div class="card">
      ...
      <div class="quick-item" style="margin-top: 12px;">
        ...
        <div class="quick-text" id="namePreview"><?= e(($category['name'] ?? '') ?: 'Tên danh mục') ?></div>
      </div>

      <div class="row gap mt-16">
        <button class="btn btn-primary btn-lg" type="submit"><?= e($submitLabel) ?></button>
        <a class="btn btn-lg" href="<?= e(url('seller', 'categories')) ?>">Hủy</a>
      </div>
    </div>
  </form>
</section>
...
<script>
  ...
  nameInput.addEventListener('input', () => {
    namePreview.textContent = nameInput.value.trim() || 'Tên danh mục';
  });
